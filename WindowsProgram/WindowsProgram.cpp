...
{
	const COORD WND_SIZE = getSize(hWnd);
	const float BLOCK_WIDTH = (WND_SIZE.X / (float)X_LENGTH);
	const float BLOCK_HEIGTH = (WND_SIZE.Y / (float)Y_LENGTH);

	unsigned int startX = 0;
	unsigned int startY = 0;
	unsigned int endX = WND_SIZE.X;
	unsigned int endY = startY;

	for (int i = 0; i < Y_LENGTH; ++i)
	{
		startY = BLOCK_HEIGTH * i;
		endY = startY;
...
void CheckClick(HWND hWnd, int x, int y)
{
	...
	int xi = x / BLOCK_WIDTH;
	int yi = y / BLOCK_HEIGTH;

	g_isClicked[yi][xi] ^= true;
}
